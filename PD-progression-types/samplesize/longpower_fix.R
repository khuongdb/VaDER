# Code copied from https://github.com/mcdonohue/longpower/blob/master/R/lmmpower.R at 2023/06/01
# this fixes the following issue: https://github.com/mcdonohue/longpower/issues/6
...
# 
# Original code from mcdonohue with bugfix:
  ...
  ...)
{
  ...
  if (sum(sapply(list(n, delta, power, sig.level), is.null)) != 1) 
    stop("aa exactly one of 'n', 'delta', 'power', and 'sig.level' must be NULL")
  ...
  # here I applied the correction: n=NULL -> n=n
  if(!is.null(results$delta.CI)){
    n.upper <- switch(method,
      hu = hu.mackey.thomas.linear.power(n=n, results$delta.CI[1], t=t, 
        sig2.i=sig2.i, cor.s.i=cor.s.i, sig2.s=sig2.s, sig2.e=sig2.e, 
        p=p, sig.level=sig.level, power=power,
        alternative=alternative, tol=tol,...)$n,
      edland = edland.linear.power(n=n, results$delta.CI[1], t=t, 
        sig2.int=sig2.i, sig2.s=sig2.s, sig2.e=sig2.e, 
        sig.level=sig.level,
        power=power,
        alternative=alternative,tol=tol,...)$n,
      diggle = diggle.linear.power(n=n, results$delta.CI[1], t=t, R=R, 
        sig.level=sig.level,
        power=power,
        alternative=alternative,tol=tol,...)$n,
      liuliang = liu.liang.linear.power(n=n, results$delta.CI[1], u=u, v=v, R=R, 
        sig.level=sig.level,
        power=power,tol=tol,...)$n)
    n.lower <- switch(method,
      hu = hu.mackey.thomas.linear.power(n=n, results$delta.CI[2], t=t, 
        sig2.i=sig2.i, cor.s.i=cor.s.i, sig2.s=sig2.s, sig2.e=sig2.e, 
        p=p, sig.level=sig.level, power=power,
        alternative=alternative,tol=tol,...)$n,
      edland = edland.linear.power(n=n, results$delta.CI[2], t=t, 
        sig2.int=sig2.i, sig2.s=sig2.s, sig2.e=sig2.e, 
        sig.level=sig.level, power=power,
        alternative=alternative,tol=tol,...)$n,
      diggle = diggle.linear.power(n=n, results$delta.CI[2], t=t, R=R, 
        sig.level=sig.level,
        power=power,tol=tol,...)$n,
      liuliang = liu.liang.linear.power(n=n, results$delta.CI[2], u=u, v=v, R=R, 
        sig.level=sig.level,
        power=power,tol=tol,...)$n)
